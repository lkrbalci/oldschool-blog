...
const AvatarWithAnime = ({ logo }: Props) => {
  ...
  if (randomValue) {
    showSpeech = randomValue < 2 ? "hidden" : "";
  }
  ...
  return (
    <div className="relative h-6 w-6 overflow-visible hover:scale-150 hover:cursor-pointer sm:h-8 sm:w-8 md:mt-16 md:h-24 md:w-24">
      <Link href={"/"}>
        <Image
          className={`bg-blue-500 h-16 w-16 rounded-full ${randomValue === 0 && "animate-wiggle"} ${randomValue === 1 && "animate-bounce"}`}
          src={
            `${keys.API_URL}${logo?.data.attributes.url}` || "/rootifera.png"
          }
          alt="Logo Image"
          fill
        />
      ...
      >
        ...
        {randomValue === 3 && <p>His name was Robert Paulson!</p>}
        {randomValue === 4 && <p>God save the King!</p>}
      </div>
    </div>
  );
};
